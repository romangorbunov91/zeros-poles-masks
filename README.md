...
$$\begin{equation}
    H(s) = G \frac{\sum_{m=1}^{M} \left(1 + \frac{s}{\omega_{z(m)}} \right)}{s^K \sum_{n=1}^{N} \left(1 + \frac{s}{\omega_{p(n)}}\right)},
\end{equation}$$

где
- ${G}$ - пропорциональный коэффициент; принят ${G=1}$,
- ${K}$ - количество интеграторов,
- ${\omega_{z(m)} \in \{\omega_{z(1)}, \omega_{z(2)}, ..., \omega_{z(M)}\}}$ - часоты нулей передаточной функции ${H(s)}$ в количестве ${M}$,
...
- `freq_tensor` - тензор размера `(length,)`; в обучении автоинкодера не требуется.
